...
RegisterNetEvent("guille_doorlock:client:setUpDoor", function()
    ...
    QBCore['UI']['Menu']['Open']('default',GetCurrentResourceName(),"menu_door",
    { 
    title = "Door menú", 
    align = "bottom-right", 
    elements = elements, 
        ...
        if v == "doortype" then
            QBCore['UI']['Menu']['Open']('default',GetCurrentResourceName(),"menu_door2",
            { 
            title = "Type menú", 
            align = "bottom-right", 
            elements = {
        ...
        elseif v == "doordist" then
            QBCore['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'dist_to_door', {
                title = 'Distance to door'
            }, function(data2, menu2)
                local dist = tonumber(data2['value'])
                if dist == nil then
                    QBCore['Functions']['Notify']('Invalid distance, try again')
                else
                    menu2['close']()
                    _distToDoor = dist
                    ExecuteCommand("door")
        ...
        elseif v == "addjob" then
            QBCore['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'new_job', {
                title = 'Introduce the job name'
            }, function(data2, menu2)
                local job = data2['value']
                if job == nil then
                    QBCore['Functions']['Notify']('Invalid job, try again')
                else
                    menu2['close']()
...
RegisterNetEvent("guille_doorlock:client:deleteDoor", function()
    _CreateThread(function()
        while true do
            ...
            QBCore['Functions']['ShowHelpNotification']("Press ~INPUT_CONTEXT~ to remove the door")
            ...
            if IsControlJustPressed(1, 38) then
                ...
                if _found then
                    QBCore['Functions']['Notify']("The door was deleted")
                    break
                else
                    QBCore['Functions']['Notify']("The door wasn't deleted")
                end
            end
            _Wait(_wait)
        end
        ...
        _CreateThread(function()
            while true do
                ...
                if IsEntityAnObject(entity) then
                    QBCore['Functions']['ShowHelpNotification']("Press ~INPUT_CONTEXT~ to add the door")
...
RegisterCommand("lockdoor", function()
    if show then
        ...
        if not allowed then
            if pin then
                QBCore['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'intr_pin', {
